...
#
# This file is part of obesityanalysis
#
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "obesityanalysis")
  tcosOfInterest <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  pathToCsv <- system.file("settings", "CohortsToCreate.csv", package = "obesityanalysis")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(name, ".json"), package = "obesityanalysis")
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "obesityanalysis")
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "obesityanalysis")
